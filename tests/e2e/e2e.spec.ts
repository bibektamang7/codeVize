...
test.describe("GitHub SaaS Application E2E Tests", () => {
	...
	test("Upgrade FREE → PRO via Khalti", async ({ page }) => {
		...
		const subscriptionURL = `**/dashboard/subscription`;
		let isUpgradeHandledInternally = false;

		try {
			await page.waitForURL(subscriptionURL, { timeout: 3000 });
			isUpgradeHandledInternally = true;
			console.log(
				"✓ Success! Upgrade handled internally. Skipping Khalti flow."
			);
		} catch (e) {
			isUpgradeHandledInternally = false;
			console.log(
				"✗ Upgrade did not redirect to dashboard. Proceeding with Khalti flow."
			);
		}

		if (!isUpgradeHandledInternally) {
		...
		await page.waitForURL("**/dashboard/subscription", { timeout: 10000 });
