...
describe("Plan Controller", () => {
  ...
  describe("createPlan", () => {
    it("should create a new plan", async () => {
      ...
      const req = mockRequest({}, { name: "Premium", price: 29.99, maxRepos: 10, description: "Premium plan" });
      const res = mockResponse();

      await createPlan(req, res, mockResponse());

      expect(mockPrisma.plan.create).toHaveBeenCalledWith({
        data: { name: "Premium", price: 29.99, maxRepos: 10, description: "Premium plan" },
      });
  ...
  describe("getAllPlans", () => {
    it("should return all plans ordered by price", async () => {
      const mockPlans = [
        { id: "free-plan", name: "Free", price: 0, maxRepos: 1, description: "Free plan" },
        { id: "premium-plan", name: "Premium", price: 29.99, maxRepos: 10, description: "Premium plan" },
      ];
  ...
  describe("updatePlan", () => {
    ...
    it("should update only provided fields", async () => {
      const mockUpdatedPlan = {
        ...
        maxRepos: 15, // updated
        description: "Premium plan", // unchanged
      };
